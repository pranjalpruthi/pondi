...
const EventCarousel = () => {
  ...
  return (
    <div className="w-full h-full bg-background rounded-2xl shadow-lg overflow-hidden relative">
...
const VirtualizedEventList = ({
  ...
  }, [events, rowVirtualizer.getVirtualItems(), onVisibleMonthChange]);
...
const UpcomingEventsList = () => {
  ...
  useEffect(() => {
    const fetchAllUpcomingEvents = async () => {
      ...
      try {
        const today = new Date();
        const fetchPromises: Promise<CalendarDay[]>[] = [];
        ...
        for (let i = 0; i < 3; i++) {
          ...
          const promise = fetch(`/api/calendar?location_city=Pondicherry&year=${year}&month=${month}`)
            .then(res => {
              if (!res.ok) {
                if (res.status === 404) return { data: [] }; // Gracefully handle no data for a future month
                throw new Error(`Failed to fetch data for ${year}-${month}`);
              }
              return res.json();
            })
            .then((result: CalendarResponse) => result.data || []);
          fetchPromises.push(promise);
        }

        const allMonthsData = await Promise.all(fetchPromises);
        const flattenedData = allMonthsData.flat();
        const processedEvents = filterAndSortUpcomingEvents(flattenedData);
