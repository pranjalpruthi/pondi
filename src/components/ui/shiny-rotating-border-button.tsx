...
export function ShinyRotatingBorderButton({
	...
	disabled,
	...props
}: Readonly<{ children: ReactNode; className?: string; disabled?: boolean }> &
		...
		>
			...
			<span className="relative z-10 block rounded-full bg-white px-6 py-3 dark:bg-black">
				<motion.span
					...
					className={cn(
						"block transform-gpu bg-clip-text text-2xl md:text-3xl lg:text-4xl font-bold tracking-tighter",
						disabled ? "text-neutral-400" : "text-transparent"
					)}
					transition={{
						duration: disabled ? 0 : 1,
						ease: "linear",
